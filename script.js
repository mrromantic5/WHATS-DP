class WhatsDPDownloader {
    constructor() {
        this.elements = {
            ...
            countrySelectorTrigger: document.getElementById('countrySelectorTrigger'),
            selectedCountryBadge: document.getElementById('selectedCountryBadge'),
            pwaInstallBtn: document.getElementById('pwaInstallBtn'),
            pwaGuideModal: document.getElementById('pwaGuideModal'),
            pwaGuideModalOverlay: document.getElementById('pwaGuideModalOverlay'),
            pwaGuideModalClose: document.getElementById('pwaGuideModalClose'),
            pwaGuideCloseBtn: document.getElementById('pwaGuideCloseBtn'),
            pwaInstallFromGuide: document.getElementById('pwaInstallFromGuide'),
            pwaUpdateModal: document.getElementById('pwaUpdateModal'),
            pwaUpdateModalOverlay: document.getElementById('pwaUpdateModalOverlay'),
            pwaUpdateModalClose: document.getElementById('pwaUpdateModalClose'),
    ...
    initPWA() {
        ...
        window.addEventListener('beforeinstallprompt', (e) => {
            ...
            if (!this.pwa.isInstalled && !this.pwa.isStandalone) {
                setTimeout(() => {
        ...
        // Show app badge if running as standalone
    ...
    setupPWAModalEvents() {
        // PWA Guide Modal
        const guideModal = this.elements.pwaGuideModal;
        const guideOverlay = this.elements.pwaGuideModalOverlay;
        const guideCloseBtn = this.elements.pwaGuideModalClose;
        const guideCloseBtn2 = this.elements.pwaGuideCloseBtn;
        const installFromGuideBtn = this.elements.pwaInstallFromGuide;
        
        // Platform selector
        const platformBtns = document.querySelectorAll('.guide-platform-btn');
        platformBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.getElementById(`guide${platform.charAt(0).toUpperCase() + platform.slice(1)}`).classList.add('active');
            });
        });
        ...
        const updateOverlay = this.elements.pwaUpdateModalOverlay;
        const updateCloseBtn = this.elements.pwaUpdateModalClose;
    ...
    showOfflineIndicator() {
        const indicator = this.elements.offlineIndicator;
        if (indicator) {
    ...
    setupCountrySelector() {
        ...
        // Add click event to country selector trigger
        this.elements.countrySelectorTrigger.addEventListener('click', () => this.openCountryModal());
    }
    
    updateCountrySelectorUI() {
        const country = this.state.selectedCountry;
        const trigger = this.elements.countrySelectorTrigger;
        const badge = this.elements.selectedCountryBadge;
        ...
        // Update input padding based on screen size
        const input = this.elements.whatsappNumber;
        if (window.innerWidth <= 480) {
            input.style.paddingLeft = '6.5rem';
            input.style.paddingRight = '2.5rem';
        } else if (window.innerWidth <= 768) {
            input.style.paddingLeft = '8rem';
            input.style.paddingRight = '3rem';
        } else {
            input.style.paddingLeft = '10.5rem';
            input.style.paddingRight = '3.5rem';
        }
    }
    
    openCountryModal() {
        this.elements.countrySelectorTrigger.classList.add('active');
    ...
    closeCountryModal() {
        this.elements.countrySelectorTrigger.classList.remove('active');
    ...
    async fetchLocationInfo(ip) {
        try {
            ...
            // Silently fail for location info
        }
    }
    ...
    setupEventListeners() {
        ...
        // Window resize handler for country selector
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                if (this.elements.countryModal.classList.contains('active')) {
                    ...
                    this.closeFollowPopup();
                } else if (this.elements.pwaGuideModal.classList.contains('active')) {
                    this.closePWAInstallGuide();
                } else if (this.elements.pwaUpdateModal.classList.contains('active')) {
                    this.closePWAUpdateModal();
                }
            }
        });
